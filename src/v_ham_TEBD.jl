...
function exp_xHB_comp(ab, x, s_total)

    #s_total = [(n == S_pos_t) | (n == S_pos_r) ? Index(2, "S=1/2") : Index(boson_dim, "Qudit") for n = 1:tot_chain]
    #s_tot_comp = s_total[S_pos_r:tot_chain]
    N_chain = Int(length(s_total)/2)

    gates = ITensor[]
    i = 1
    for j in 2:N_chain-1
        s1 = s_total[N_chain+j]
        s2 = s_total[N_chain+j+1]
        s1p = s_total[N_chain+1-j]
        s2p = s_total[N_chain-j]
        ...
        hj = ω_n * op("N", s1) * op("Id", s2) +
            ...
            - ω_n * op("N", s1p) * op("Id", s2p) 
            - t_n * op("Adag", s1p) * op("A", s2p) 
            - t_n * op("A", s1p) * op("Adag", s2p)
...
##########################################################################
function tot_gate(ω_0, Ω, c_0, ab, tau, s_total)
    ...
    for j in 1:N_chain-1
        ...
        if j == 1
            hj = ω_0 * op("Sz", s1) * op("Id", s2)  +
                 ...
                 -c_0 * op("Sz", s1p) * op("A", s2p) 
                 -c_0 * op("Sz", s1p) * op("Adag", s2p)
        else
            ω_n = ab[i-1, 1]
            t_n = sqrt(ab[i, 2])
            hj = ω_n * op("N", s1) * op("Id", s2)  +
