...
N_chain = 22           ## Number of chain sites for single chain-transformed environment
...
Threads.@threads for α in α_list
    ...
    for t in 1:nt
        ...
        @show t*tau
        chi = tr(cd_U_ρ_Ud)
        println(real(imag.(chi[1])/u)-f(t*tau))
...
#end
#= if ω_0 == 1 
    plot!((collect(0:1:length(mean_Q)-1)*tau)[begin:10:end],f.(collect(0:1:length(mean_Q)-1)*tau)[begin:10:end],label= "T=$T, α = $α, exact")
end =#

plot!(legend=:bottomright)
xlabel!("t")  
title = string(L"\langle Q \rangle, N_{ch} = %$(N_chain), d = %$n1_bsn_dim, \mathrm{cutoff} = 10^{%$(cut)}, \Delta t = %$tau, u = %$u")
title!(title)
display("image/png", p)
