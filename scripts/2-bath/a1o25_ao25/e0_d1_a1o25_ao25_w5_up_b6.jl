...
let
	...
	n1_bsn_dim = 6  # Dimension of chain sites
	...
	push!(mean_Q2, mQ2)
	push!(var_Q2, vQ2) =#
	write_for_loop(file_name_txt_m, string(1), "$(model) boson: T = [$T1, $T2], alpha = [$α1, $α2], N_chain = $N_chain, maxdim = $maxdim, cutoff = $cut, tau = $tau, jump = $jump, boson_dim = $n1_bsn_dim, omega = $ω_C")
	write_for_loop(file_name_txt_v, string(1), "$(model) boson: T = [$T1, $T2], alpha = [$α1, $α2], N_chain = $N_chain, maxdim = $maxdim, cutoff = $cut, tau = $tau, jump = $jump, boson_dim = $n1_bsn_dim, omega = $ω_C")
	write_for_loop(file_name_txt_N, string(1), "$(model) boson: T = [$T1, $T2], alpha = [$α1, $α2], N_chain = $N_chain, maxdim = $maxdim, cutoff = $cut, tau = $tau, jump = $jump, boson_dim = $n1_bsn_dim, omega = $ω_C")
	...
	for t in 1:nt
		...
		if t % jump == 0
			...
			write_for_loop(file_name_txt_N, string(t + 1), string(n_list))
		end
	end
	...
	write_to_file(file_name_txt_v, "$(model) boson: T = [$T1, $T2], alpha = [$α1, $α2], N_chain = $N_chain, maxdim = $maxdim, cutoff = $cut, tau = $tau, jump = $jump, boson_dim = $n1_bsn_dim, omega = $ω_C", string(var_J))
end
