...
let
	...
	push!(mean_Q, mQ)
	push!(var_Q, vQ)
	write_for_loop(file_name_txt_m, string(1), "$(model) boson: T = $T, alpha = $α, N_chain = $N_chain, maxdim = $maxdim, cutoff = $cut, tau = $tau, u = $u, boson_dim = $n1_bsn_dim")
	write_for_loop(file_name_txt_v, string(1), "$(model) boson: T = $T, alpha = $α, N_chain = $N_chain, maxdim = $maxdim, cutoff = $cut, tau = $tau, u = $u, boson_dim = $n1_bsn_dim")
	...
	for t in 1:nt
		...
		U_ρ_Ud = normalize(ITensors.truncate(ITensors.truncate(U_ρ_Ud; maxdim = 20, site_range = (1:(Int(floor(N_chain / 2))))); maxdim = 30, site_range = (tot_chain:tot_chain-(Int(floor((N_chain / 2)))))))
	...
	write_to_file(file_name_txt_m, "$(model) boson: T = $T, alpha = $α, N_chain = $N_chain, maxdim = $maxdim, cutoff = $cut, tau = $tau, u = $u, boson_dim = $n1_bsn_dim", string(mean_Q))
	write_to_file(file_name_txt_v, "$(model) boson: T = $T, alpha = $α, N_chain = $N_chain, maxdim = $maxdim, cutoff = $cut, tau = $tau, u = $u, boson_dim = $n1_bsn_dim", string(var_Q))
end
