...
let
    ...
    ρ = MPO(state, s_total)
    ρ /= tr(ρ) # Normalize the density matrix
	...
	U_ρ_Ud = deepcopy(ρ)
	H_U_ρ_Ud = apply(heat_op, U_ρ_Ud; maxdim = 10)
	H_H_U_ρ_Ud = apply(heat_op, H_U_ρ_Ud; maxdim = 10)
	@show mQ = real(tr(H_U_ρ_Ud))
	@show vQ = real(tr(H_H_U_ρ_Ud))
	push!(mean_Q, mQ)
	push!(var_Q, vQ)
	write_for_loop(file_name_txt_m, string(1), "$(model) boson: T = $T, alpha = $α, N_chain = $N_chain, maxdim = $maxdim, cutoff = $cut, tau = $tau, boson_dim = $n1_bsn_dim")
	write_for_loop(file_name_txt_v, string(1), "$(model) boson: T = $T, alpha = $α, N_chain = $N_chain, maxdim = $maxdim, cutoff = $cut, tau = $tau, boson_dim = $n1_bsn_dim")
	...
	for t in 1:nt
		...
		U_ρ_Ud = apply(evol, U_ρ_Ud; cutoff, apply_dag = true)
		@show t
		#U_ρ_Ud = add(0.5 * swapprime(dag(U_ρ_Ud), 0 => 1), 0.5 * U_ρ_Ud; maxdim = 2 * maxdim)
	...
	write_to_file(file_name_txt_m, "$(model) boson: T = $T, alpha = $α, N_chain = $N_chain, maxdim = $maxdim, cutoff = $cut, tau = $tau, boson_dim = $n1_bsn_dim", string(mean_Q))
	write_to_file(file_name_txt_v, "$(model) boson: T = $T, alpha = $α, N_chain = $N_chain, maxdim = $maxdim, cutoff = $cut, tau = $tau, boson_dim = $n1_bsn_dim", string(var_Q))
end
