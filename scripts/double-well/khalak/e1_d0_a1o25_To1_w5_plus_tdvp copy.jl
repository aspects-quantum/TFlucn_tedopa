...
let
	...
	#stfd = vcat([(n == S_pos) ? [s_total[n],Index(2, "Fermion, dot")] : ((n in ph_pos_t) || (n in ph_pos_r)) ? [stbath[n],Index(2, "Boson")] : [stbath[n],Index(2, "Fermion, bath")] for n ∈ 1:tot_chain]...)
	#= 
	...
	U_ψ = ψ
	H_U_ψ = apply(heat_op, ψ; cutoff)
	@show mQ = real(inner(U_ψ', H_U_ψ))
	@show vQ = real(inner(H_U_ψ', H_U_ψ))
	push!(mean_Q, mQ)
	push!(var_Q, vQ - mQ^2)
	...
	for t in 1:nt
		...
		H_U_ψ = apply(heat_op, ψ; cutoff)
		@show mQ = real(inner(U_ψ', H_U_ψ))
		@show vQ = real(inner(H_U_ψ', H_U_ψ)) - mQ^2
