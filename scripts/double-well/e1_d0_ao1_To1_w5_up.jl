...
let
    ...
    n1_bsn_dim = 14
    b_dim_real = [n1_bsn_dim - round(Int64, (n1_bsn_dim - 2.6) * (i - 1) / (N_chain - 1)) for i in 1:N_chain]  # Dimension of chain sites
    ...
    for t in 1:nt
        ...
        U_ρ_Ud = ITensors.truncate(ITensors.truncate(U_ρ_Ud; maxdim=10, site_range=(1:(Int(floor(3 * N_chain / 4))))); maxdim=15, site_range=(tot_chain:tot_chain-(Int(floor((3 * N_chain / 4))))))
