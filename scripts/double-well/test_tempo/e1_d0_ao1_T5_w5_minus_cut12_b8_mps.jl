...
ITensors.state(::StateName"minus", ::SiteType"S=1/2") = (1 / sqrt(2)) * [1; -1]  # Density matrix for qudit
...
let
	...
	jump = 4  # Number of time steps for each evolution
	...
	state = [(n == S_pos) ? "minus" : "0" for n in 1:tot_chain]
	ψ = MPS(s_total, state)
