...
#N_chain = 214           ## Number of chain sites for single chain-transformed environment
N_chain = 50           ## Number of chain sites for single chain-transformed environment
...
U_ρ_Ud1 = ρ
cd_U_ρ_Ud1 = apply(count_gates, ρ; cutoff, maxdim)
...
p = plot()
plot!((collect(0:1:length(var_Q)-1)*tau)[begin:10:end],var_Q[begin:10:end],label= "T=$T, α = $α")

plot!(legend=:bottomright)
xlabel!("t")  
title = string(L"\langle\langle Q^2 \rangle\rangle, N_{ch} = %$(N_chain), d = %$n1_bsn_dim, \mathrm{cutoff} = 10^{%$(cut)}, \Delta t = %$tau, u = %$u")
title!(title)
display("image/png", p)
