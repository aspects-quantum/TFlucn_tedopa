...
let
    ...
    count_gates = exp_xHB(ab1, ab2, im * u / 2, s_total)
    count_dates = exp_xHB(ab1, ab2, -im * u / 2, s_total)
    ...
    U_ρ_Ud = copy(ρ)
    cd_U_ρ_Ud1 = apply(count_dates, U_ρ_Ud; cutoff=.1*cutoff)
    cd_U_ρ_Ud = apply(count_gates, swapprime(dag(cd_U_ρ_Ud1), 0 => 1); cutoff=.1*cutoff)
    chi = tr(cd_U_ρ_Ud)
    push!(char_fn, chi[1])
