...
let
	...
	β = 1 / T
	u = 0.002  # Counting field parameter
	...
	U_ρ_Ud = copy(ρ)
	cd_U_ρ_Ud = apply(count_gates, U_ρ_Ud; cutoff; cutoff = cutoff * 0.01)
	chi = tr(cd_U_ρ_Ud)
	push!(char_fn, chi[1])
	...
	for t in 1:nt
		...
		normalize!(ITensors.truncate(ITensors.truncate(U_ρ_Ud; maxdim = 20, site_range = (1:(Int(floor(N_chain / 3))))); maxdim = 30, site_range = (tot_chain:tot_chain-(Int(floor((N_chain / 3)))))))
		...
		if t % 150 == 0
			...
			safesave("links_mQ_ub_ao25.png", p2)
		end
	...
	p = plot(t_list, real(mean_Q), label = "α = $α")
