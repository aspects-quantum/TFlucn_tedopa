...
N_chain = 89           ## Number of chain sites for single chain-transformed environment
...
n1_bsn_dim = 5;
b_dim = [n1_bsn_dim for i = 1:N_chain]   #-round(Int64,(n1_bsn_dim-2.6)*(i-1)/(N_chain-1))        ## Dimension of chain sites
...
w_fn1(k) = ω_C^2 * (2*α*k*exp(-k))*(1 + n(k))         ## weight function for real space bath
w_fn2(k) = ω_C^2 * (2*α*k*exp(-k))*(n(k))                ## weight function for tilde space bath
