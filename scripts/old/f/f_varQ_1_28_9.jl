...
write_to_file(file_name_txt, "Independent boson: T = $T, alpha = $α, N_chain = $N_chain, maxdim = $maxdim, cutoff = $cut, tau = $tau", string(var_Q))
...
xlabel!("t")  
title = string(L"\langle\langle Q^2 \rangle\rangle, N_{ch} = %$(N_chain), \beta = %$β, \mathrm{cutoff} = 10^{%$(cut)}, \Delta t = %$tau")
title!(title)
display("image/png", p)
file_name_png = string(split(split(@__FILE__, ".")[end-1], string(\))[end],"_T=$T--α=$α--Ω=$Ω.png")
