...
#maxdim_ops = 5
maxdim = 1000
tau = .05             ## time step duration
nt = 300
ttotal = nt * tau           ## TOTAL TIME evolution

N_chain = 170           ## Number of chain sites for single chain-transformed environment
...
@show real(mean_Q)
write_to_file(file_name_txt, "$(model) boson: T = $T, alpha = $α, N_chain = $N_chain, maxdim = $maxdim, cutoff = $cut, tau = $tau", string(real(mean_Q)))
...
xlabel!("t")  
title = string(L"\langle Q \rangle, N_{ch} = %$(N_chain), \beta = %$β, \mathrm{cutoff} = 10^{%$(cut)}, \Delta t = %$tau")
title!(title)
display("image/png", p)
