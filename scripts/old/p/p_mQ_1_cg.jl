...
N_chain = 180           ## Number of chain sites for single chain-transformed environment
...
n1_bsn_dim = 10
b_dim = [n1_bsn_dim - round(Int64, (n1_bsn_dim - 3.6) * (i - 1) / (N_chain - 1)) for i in 1:N_chain]  ## Dimension of chain sites
...
u = 0.003 # counting field parameter

p=pgfplotsx()  # Use PGFPlotsX backend for publication-quality plots
...
Threads.@threads for α in α_list
    mean_Q = Float64[]
    ...
    for t in 1:nt
        ...
        cd_U_ρ_Ud = apply(count_gates, U_ρ_Ud; cutoff, maxdim)
#=         cd_U_ρ_Ud = apply(cd_U_ρ_Ud1, count_gates; cutoff, maxdim)
 =#
        @show t * tau
    ...
    global p=plot!(t_list, real(mean_Q), label = "α = $α")
...
default(
    ...
    legendfontsize = 50,     # Small legend font for clarity
