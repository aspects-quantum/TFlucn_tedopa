...
r0_mps = convert(MPS, I0)
for i = 1:Int(length(s_tot_comp))
    r0_mps[i] = r0_mps[i]*delta(s_tot_comp[i]', s_total[S_pos_t+1-i])
end
normalize!(r0_mps)
#@show r0_mps

#r0_mps = tdvp(hbb_comp, -0.5*β, r0_mps; nsteps=10, updater_backend="exponentiate", cutoff=1e-7, maxdim=10, normalize=true)
@show r0_mps = apply(exp_xHB_tdvp(ab1, -0.5*β/Nbeta, s_tot_comp), r0_mps; cutoff, maxdim)
for i = 2:Nbeta           ## perform IMAGINARY TIME evolution
    global r0_mps = apply(exp_xHB_tdvp(ab1, -0.5*β/Nbeta, s_tot_comp), r0_mps; cutoff, maxdim)
    normalize!(r0_mps)
...
cg_ρ = tdvp(bath_ham, -u*1.0im, ρ_vec; updater_backend="exponentiate", cutoff, maxdim, normalize=true)
U_cg_ρ = cg_ρ
U_ρ = ρ_vec
cg_U_ρ = cg_ρ
...
for t in 1:nt
    global U_cg_ρ = tdvp(tot_ham, -1.0im*tau, U_cg_ρ; updater_backend="exponentiate", cutoff, maxdim, normalize=true)
    @show U_cg_ρ
    global U_ρ = tdvp(tot_ham, -1.0im*tau, U_ρ; updater_backend="exponentiate", cutoff, maxdim, normalize=true)
    @show U_ρ
    global cg_U_ρ = tdvp(bath_ham, -1.0im*u, U_ρ; updater_backend="exponentiate", cutoff, maxdim, normalize=true)
    @show cg_U_ρ

    chi = inner(cg_U_ρ, U_cg_ρ)
    println(chi[1])
    write_for_loop("tdvp_1.txt", string(t), string(chi[1]))
    push!(char_fn,chi[1])
end
chi_pu = char_fn
