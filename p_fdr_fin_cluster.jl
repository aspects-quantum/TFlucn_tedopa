...
Threads.@threads for T_idx in 1:length(T_list)
        ...
        tau = 0.001*10/T  # Time step duration
        u = 2*tau
        ...
        while t < 2
                ...
                if i%10 == 0
                        ...
                        U_ρ_Ud = normalize(ITensors.truncate(ITensors.truncate(U_ρ_Ud; maxdim = 15, site_range = (1:(Int(floor(N_chain / 2))))); maxdim = 20, site_range = (tot_chain:tot_chain-(Int(floor((N_chain / 2)))))))
                end
...
@show FDR
write_to_file(file_name_txt, "$(model) boson: T = $(T_list), alpha = $α, N_chain = $N_chain, maxdim = $maxdim, cutoff = $cut, boson_dim = $n1_bsn_dim, FDR = ", string(real(FDR)))
write_to_file(file_name_txt, "$(model) boson: T = $(T_list), alpha = $α, N_chain = $N_chain, maxdim = $maxdim, cutoff = $cut, boson_dim = $n1_bsn_dim, vQ = ", string(real(vQ)))
write_to_file(file_name_txt, "$(model) boson: T = $(T_list), alpha = $α, N_chain = $N_chain, maxdim = $maxdim, cutoff = $cut, boson_dim = $n1_bsn_dim, mQ = ", string(real(mQ)))