...
xticks1 = (0:1:4) #range(2, stop = 15, length = 3)
xticks2 = (0:1:2)
xticks3 = (0:1:2) #range(5, stop = maximum(time_steps[end-1]), length = 3)
xticks3sub = (10:4:15) #range(10, stop = maximum(time_steps[end-1]), length = 2)

yticks1 = (0:10:30) #range(0, stop = 6, length = 3)
yticks2 = (0:200:700) #range(0, stop = 30, length = 3)
...
scatter!(time_steps[1:gap:length(vQ2)], vQ2[1:gap:end] ./ mQ2[1:gap:end], markersize=markersize, color = :teal, label = "")
scatter!(time_steps[1:gap:length(vQ3)], vQ3[1:gap:end] ./ mQ3[1:gap:end], markersize=markersize, markerstrokewidth=.6, color = :lightcoral, label = "")
scatter!(time_steps[1:gap:length(vQ4)], vQ4[1:gap:end] ./ mQ4[1:gap:end],markersize=markersize, markerstrokewidth=.6, color = :brown, label = "")
plot!(widen = false)
ylims!(0, 200)  
xlims!(0, 2.8)  
...
plot!(legendtitle=L"\ \ \ \ \alpha_1 \ \ \ \ \ \alpha_2", legendtitlefontsize=12,legend = (.5,.63),legendfontsize = 12)
